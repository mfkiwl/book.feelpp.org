...
tic()
app = fppc.Environment(["myapp"],config=fppc.localRepository("")) # <1>
toc("init Feel++")

logger.info(f"Feel++ version: {fppc.Info.version()}")
print("Feel++ version: ",fppc.Info.version())
----
...
geofilename=fppc.download( "github:{repo:feelpp,path:feelpp/quickstart/laplacian/cases/feelpp2d/feelpp2d.geo}", worldComm=app.worldCommPtr() )[0]

tic()
mesh = fppc.load(fppc.mesh(dim=2,realdim=2), name=geofilename, h=0.1, verbose=1) # <2>
toc("load mesh")
logger.info(f"mesh loaded")

Xh=fppc.functionSpace(mesh=mesh,space="Pch",order=1) # <3>
P0h=fppc.functionSpace(mesh=mesh,space="Pdh",order=0) # <4>
u=Xh.element() # <5>
u.on(range=fppc.elements(mesh), expr=fppc.expr("sin(2*pi*x)*cos(pi*y):x:y")) # <6>
logger.info(f"functionspace built and u created")

e = fppc.exporter(mesh=mesh)  # <7>
e.add("a_scalar", 1.) # <8>
e.add("u",u) # <9>
e.add("pid",fppc.pid( P0h )) # <10>
e.save() # <11>
